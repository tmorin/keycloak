    ...
    protected abstract class Binding {
        ...
        protected Response handleSamlArt(String samlArt, String relayState, String clientId) {
            try {
                ...
                // validate the destination of the ArtifactResponse
                if (isDestinationRequired() && artifactResponse.getDestination() == null && containsUnencryptedSignature(samlDocumentHolder)) {
                    logger.error("artifact binding failed: the ArtifactResponse does not contain a destination");
                    event.event(EventType.IDENTITY_PROVIDER_RESPONSE);
                    event.detail(Details.REASON, Errors.MISSING_REQUIRED_DESTINATION);
                    event.error(Errors.INVALID_SAML_RESPONSE);
                    return ErrorPage.error(session, null, Response.Status.BAD_REQUEST, Messages.INVALID_REQUEST);
                }
                if (!destinationValidator.validate(getExpectedDestination(config.getAlias(), clientId), artifactResponse.getDestination())) {
                    logger.error("artifact binding failed: the ArtifactResponse has an invalid destination");
                    event.event(EventType.IDENTITY_PROVIDER_RESPONSE);
                    event.detail(Details.REASON, Errors.INVALID_DESTINATION);
                    event.error(Errors.INVALID_SAML_RESPONSE);
                    return ErrorPage.error(session, null, Response.Status.BAD_REQUEST, Messages.INVALID_REQUEST);
                ...
                logger.debug(String.format("embeddedResponseAsString %s", embeddedResponseAsString));
        ...
        protected Response handleLoginResponse(String samlResponse, SAMLDocumentHolder holder, ResponseType responseType, String relayState, String clientId) {
            ...
            try {
                ...
                boolean isArtifactBinding = "true".equals(authSession.getClientNote(JBossSAMLURIConstants.SAML_HTTP_ARTIFACT_BINDING.get()));
